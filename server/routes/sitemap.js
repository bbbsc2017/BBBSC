...
const staticPaths = [
  ...
  '/portugal', '/winter-work-alemania', '/work-and-travel-usa/inscripcion', '/asia/inscripcion', '/winter-work-alemania/inscripcion', '/programas-academicos', '/alemania', '/polonia', '/canada',
  '/corea-del-sur', '/usa', '/australia', '/trabaja-con-nosotros', '/hunters', '/journey-begins-bucaramanga', '/terminos-y-condiciones', '/blog', '/ofertas',
...
sitemapRouter.get('/sitemap.xml', (_req, res) => {
  ...
  const offers = db.prepare("SELECT slug, sponsor, employer, updated_at FROM job_offers WHERE status != 'draft' ORDER BY updated_at DESC").all()
  ...
  const entries = [
    ...
    ...offers.map((offer) => urlEntry(`/ofertas/${slugify(offer.sponsor)}/${slugify(offer.employer)}/${offer.slug}`, offer.updated_at, 'daily', '0.8')),
  ]
