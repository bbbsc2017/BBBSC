...
export function ProgramHero({ eyebrow, title, description, country, image, requirements, breadcrumbs, primaryTo, primaryLabel, secondaryTo = '#contenido-programa', secondaryLabel = 'Conoce el programa', requirementsLabel = 'Requisitos principales', itemLabel = 'Requisito', showRequirements = true, compact = false }: ProgramHeroProps) {
  const heroHeight = compact ? 'min-h-[500px] sm:min-h-[560px] lg:min-h-[560px]' : 'min-h-[660px] sm:min-h-[720px] lg:min-h-[680px]'
  return <section className="relative bg-transparent pb-8 pt-6 sm:pb-12 sm:pt-8">
    <Container>
      <Breadcrumbs items={breadcrumbs} />
      <div className={`relative mt-5 ${heroHeight} overflow-hidden rounded-[2rem] border border-white/10 bg-[#1c1c1c] shadow-2xl shadow-[#1c1c1c]/70`}>
        ...
        <div className={`relative z-10 flex ${heroHeight} flex-col px-5 py-7 sm:px-9 sm:py-9 lg:px-14 lg:py-12`}>
          <div className={`max-w-2xl lg:my-auto ${showRequirements ? 'lg:pb-32' : ''}`}>
          ...
          {showRequirements && <div className="mt-auto border-t border-white/15 pt-5 sm:pt-6">
