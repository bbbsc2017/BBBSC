...
export function ProgramFAQ({ items, minimal = false }: ProgramFAQProps) {
  return (
    <div>
      <h3 className="mb-4 text-base font-bold text-white">Preguntas frecuentes</h3>
      <div className={minimal ? "space-y-3" : "divide-y divide-white/10 rounded-3xl border border-white/10 bg-ink-800"}>
        {items.map((item) => (
          <details key={item.question} className={`group transition duration-300 ${minimal ? 'rounded-2xl bg-white/[.035] px-5 py-4 open:pb-5 hover:bg-white/[.075]' : 'px-5 py-4 open:pb-5'}`}>
