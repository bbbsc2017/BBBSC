  ...
  return (
    <section className="relative py-20 sm:py-28">
      <Container className="flex flex-col items-center gap-14">
        ...
        <div className="w-full max-w-3xl divide-y divide-ink/10 rounded-3xl border border-ink/10 bg-white">
          {faqItems.map((item) => (
            <details key={item.question} className="group px-6 py-5 open:pb-6">
              <summary className="flex cursor-pointer list-none items-center justify-between gap-4 text-left text-base font-semibold text-ink">
                {item.question}
                <Plus className="size-5 shrink-0 text-brand-700 transition-transform duration-300 group-open:rotate-45" />
              </summary>
